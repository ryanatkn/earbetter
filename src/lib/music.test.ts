...
import {lookup_scale, to_scale_notes} from '$lib/music.js';

test('to_scale_notes', () => {
	assert.deepEqual(to_scale_notes(lookup_scale('major pentatonic'), 1), [2, 4, 7, 9, 12]);
	assert.deepEqual(
	...
	assert.deepEqual(to_scale_notes(lookup_scale('minor'), 1), [2, 3, 5, 7, 8, 10, 12]);
	assert.deepEqual(
