<script lang="ts">
  ...
  */
	...
	const level = create_level(level_def, audio_ctx, volume, instrument);
...
<MidiInput
	...
	on:note_start={(e) => {
		...
		if ($status === 'complete') {
			start_playing(audio_ctx, e.detail.note, $volume, $instrument);
		} else {
		...
		{#if clientWidth}
			<Piano
				...
				on:press={$status === 'waiting_for_input'
					...
					: $status === 'complete'
					? (e) => start_playing(audio_ctx, e.detail, $volume, $instrument)
					: undefined}
